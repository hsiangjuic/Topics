<template>
        ...
        <table class="table table-bordered table-hover table-striped member-list">
          ...
          <tbody>
            <tr v-for="customer in customers" :key="customer.cId">
              ...
              <td>{{ customer.memberRank ? customer.memberRank.name : '非會員' }}</td>
              ...
              <td>
                <button @click="editCustomer(customer)" class="btn btn-danger">
  {{ customer.memberStatus === 'true' ? '禁用' : '啟用' }}
</button>&nbsp;
  ...
  // 反轉memberStatus的值
  customer.memberStatus = customer.memberStatus === 'true' ? 'false' : 'true';
  ...
  const formValues = {
    ...
    titleOfCourtesy: customer.titleOfCourtesy,
    allowPromotionMail: customer.allowPromotionMail,
    memberStatus: customer.memberStatus, // 更新memberStatus的值
    mrId: customer.mrId,
  };
